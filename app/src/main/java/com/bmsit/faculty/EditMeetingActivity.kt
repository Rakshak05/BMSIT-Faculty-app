...
class EditMeetingActivity : AppCompatActivity() {
    ...
    private lateinit var etReason: com.google.android.material.textfield.TextInputEditText
    private lateinit var etVenue: com.google.android.material.textfield.TextInputEditText
    private lateinit var actParticipants: androidx.appcompat.widget.AppCompatAutoCompleteTextView
    private lateinit var tvSelectedDate: android.widget.TextView
    private lateinit var tvYear: android.widget.TextView
    ...
    private lateinit var cardStartTime: com.google.android.material.card.MaterialCardView
    private lateinit var cardDuration: com.google.android.material.card.MaterialCardView
    private lateinit var btnUpdateMeeting: com.google.android.material.button.MaterialButton
    private lateinit var toolbarEdit: androidx.appcompat.widget.Toolbar
    ...
    private fun initViews() {
        ...
        actParticipants = findViewById(R.id.actParticipants)
    ...
    private fun loadPreviousMeetingsForSuggestions() {
        ...
        db.collection("meetings")
            ...
            .addOnFailureListener { exception ->
                Log.e("EditMeetingActivity", "Error loading previous meetings", exception)
            }
    }
    
    private fun setupAutoComplete() {
        // Note: etReason and etVenue are TextInputEditText, not AutoCompleteTextView
        // Auto-complete is not directly supported on TextInputEditText
        // We'll skip auto-complete for these fields in this implementation
    }
        ...
        db.collection("meetings").document(id).get()
            .addOnSuccessListener { document ->
                ...
                if (meeting != null) {
                    ...
                    // Fill basic fields
                    etReason.setText(meeting.title)
                    etVenue.setText(meeting.location)
                    ...
                    // Set duration
                    val durationText = when (meeting.duration) {
                        30 -> "30 min"
                        45 -> "45 min"
                        60 -> "1 hr"
                        90 -> "1.5 hrs"
                    ...
                    val mappedValue = when (meeting.attendees) {
                        "All Faculty" -> "All Faculty"
                        "Associate Prof" -> "Associate Professors"
                        "Assistant Prof" -> "Assistant Professors"
                        "HOD Only" -> "HODs Only"
                        "Custom" -> "Custom"
                        else -> meeting.attendees
                    }
                    actParticipants.setText(mappedValue, false)
            ...
            .addOnFailureListener { exception ->
                Log.e("EditMeetingActivity", "Error loading meeting data", exception)
    ...
    private fun setupDropdowns() {
        ...
        actParticipants.setAdapter(participantAdapter)
        
        // Handle participant selection changes
        actParticipants.onItemClickListener = android.widget.AdapterView.OnItemClickListener { _, _, position, _ ->
    ...
    private fun setupPickers() {
        ...
        cardDuration.setOnClickListener { view ->
    ...
    private fun updateDateTimeDisplay() {
        ...
        tvSelectedDate.text = dayFormat.format(selectedCalendar.time)
        tvYear.text = selectedCalendar.get(Calendar.YEAR).toString()
        
        // Update time display - keeping the original format you liked
        val timeFormat = SimpleDateFormat("hh:mm a", Locale.getDefault())
        tvStartTime.text = timeFormat.format(selectedCalendar.time)
    }

    private fun setupSubmitButton() {
        btnUpdateMeeting.setOnClickListener {
            val reason = etReason.text.toString()
            val venue = etVenue.text.toString()
            val participants = actParticipants.text.toString()
            ...
            db.collection("users").document(schedulerId).get()
                .addOnSuccessListener { document ->
                    if (document.exists()) {
                        ...
                        // Continue with meeting updating
    ...
    private fun updateMeeting(title: String, venue: String, group: String, duration: String) {
        btnUpdateMeeting.isEnabled = false
        btnUpdateMeeting.text = "Updating..."
        
        val schedulerId = auth.currentUser?.uid
        if (schedulerId == null) {
            Toast.makeText(this, "Error: You must be logged in.", Toast.LENGTH_LONG).show()
            btnUpdateMeeting.isEnabled = true
        ...
        // Create updated meeting object for conflict checking
        ...
        // Check for conflicts with existing meetings for ALL participants
        checkForConflicts(updatedMeeting) { hasConflicts ->
            if (hasConflicts) {
    ...
    private fun checkForConflicts(newMeeting: Meeting, callback: (Boolean) -> Unit) {
        ...
        db.collection("users").document(schedulerId).get()
                ...
                db.collection("meetings")
                    ...
                    .addOnSuccessListener { result ->
                        ...
                        for (document in result) {
                            ...
                            // Skip the meeting we're trying to update (same ID)
                        ...
                        if (conflictingMeetings.isEmpty()) {
                            ...
                            checkOverrideCapability(meetingsToCheck, schedulerId, currentUserRank) { overrideableMap ->
                                if (overrideableMap.isNotEmpty()) {
                                    ...
                                    showDetailedConflictDetails(newMeeting, mapOf("conflicts" to conflictingMeetings))
                                }
                    ...
                    .addOnFailureListener { exception ->
                        Log.e("EditMeetingActivity", "Error checking for conflicts", exception)
            ...
            .addOnFailureListener { exception ->
                Log.e("EditMeetingActivity", "Error getting user designation", exception)
                // In case of error, we'll be conservative and assume there's a conflict
    ...
    private fun showConflictDialogWithOverride(
        ...
        // Fetch user names for all conflicting meetings
        fetchUserNamesForDetailedConflicts(mapOf("conflicts" to conflictingMeetings)) { userNamesMap ->
    ...
    private fun handleOverrideActionWithName(
        ...
        batch.commit()
            .addOnSuccessListener {
                ...
                val updates = mutableMapOf<String, Any>(
                    "title" to newMeeting.title,
                    "location" to newMeeting.location,
                    "dateTime" to newMeeting.dateTime,
                    "attendees" to newMeeting.attendees,
                    "duration" to newMeeting.duration
                )
                
                // If this is a custom meeting, also update the custom attendee UIDs
                if (newMeeting.attendees == "Custom") {
                    updates["customAttendeeUids"] = newMeeting.customAttendeeUids
                }
                
                // Update the document in Firestore
                db.collection("meetings").document(newMeeting.id).update(updates)
                    .addOnSuccessListener {
                        ...
                        Toast.makeText(this, "Meeting updated successfully! ${overrideableMeetings.size} lower-ranking meeting(s) were cancelled.", Toast.LENGTH_LONG).show()
                        finish()
                    }
                    .addOnFailureListener { e ->
                        btnUpdateMeeting.isEnabled = true
    ...
    private fun sendCancellationNotifications(overrideableMeetings: Map<Meeting, String>, currentUserDesignation: String, currentUserName: String) {
        for ((meeting, schedulerName) in overrideableMeetings) {
            ...
            db.collection("notifications").add(notificationData)
                .addOnFailureListener { e ->
                    Log.e("EditMeetingActivity", "Error sending cancellation notification", e)
                }
        }
    }
    ...
    private fun sendRefreshNotification() {
        ...
        val notificationData = hashMapOf(
            ...
            "timestamp" to Timestamp(Date()),
            "message" to "Meeting updated, please refresh your dashboard"
        )
        
        db.collection("notifications").add(notificationData)
            .addOnSuccessListener {
                Log.d("EditMeetingActivity", "Refresh notification sent successfully")
            }
            .addOnFailureListener { e ->
                Log.e("EditMeetingActivity", "Error sending refresh notification", e)
            }
    }
}